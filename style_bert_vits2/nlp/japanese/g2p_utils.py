...
from style_bert_vits2.nlp.symbols import PUNCTUATIONS, DIFFICULTY


def g2kata_tone(norm_text: str) -> list[tuple[str, int, float]]:
    """
...
def phone_tone2kata_tone(phone_tone: list[tuple[str, int]]) -> list[tuple[str, int, float]]:
    """
    ...
    for phone, next_phone, tone, next_tone in zip(phones, phones[1:], tones, tones[1:]):
        ...
        if phone in CONSONANTS:  # n以外の子音の場合
            ...
            # phoneが母音もしくは「N」の場合
            difficulty = DIFFICULTY.get(current_mora, 0.0)
            current_mora += phone
            result.append((MORA_PHONEMES_TO_MORA_KATA[current_mora], tone, difficulty))
            current_mora = ""

    return result


def kata_tone2phone_tone(kata_tone: list[tuple[str, int, float]]) -> list[tuple[str, int, float]]:
    """
    ...
    result: list[tuple[str, int, float]] = [("_", 0, 0.0)]
    for mora, tone, difficulty in kata_tone:
        if mora in PUNCTUATIONS:
            result.append((mora, tone, 0.0))
        else:
            consonant, vowel = MORA_KATA_TO_MORA_PHONEMES[mora]
            if consonant is None:
                result.append((vowel, tone, 0.0))
            else:
                result.append((consonant, tone, difficulty))
                result.append((vowel, tone, 0.0))
    result.append(("_", 0, 0.0))
